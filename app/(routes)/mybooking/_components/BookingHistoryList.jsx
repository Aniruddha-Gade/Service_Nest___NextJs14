...
function BookingHistoryList({ bookingHistory, type }) {
    ...
    const cancelAppointment = (booking) => {
        GlobalApi.deleteBooking(booking.id).then(resp => {
            if (resp) {
                console.log("Booked business Successfully deleted => ", resp)
    ...
    return (
        <div className='grid grid-cols-1 md:grid-cols-2 gap-2'>
            {bookingHistory.map((booking, index) => {
                ...
                const imageUrl = booking?.businessList?.images[0]?.url ? booking.businessList.images[0].url : 'assets/images/avatar.png';
                ...
                return(
                    <div className='border rounded-lg p-4 mb-5'>
                        ...
                        {/* dialog box for delete business */}
                        <AlertDialog>
